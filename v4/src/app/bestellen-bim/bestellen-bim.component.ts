...
export class BestellenBimComponent implements OnInit {
  ...
  ngOnInit() {
    this.forgeViewerOptions = {
      // We can do this.setUrn.bind(this)  instead to expose functions of this component, the scope is normally situated at the base component.
  ...
  public fireEvent(event: ForgeViewerEvent){
    ...
    if(event instanceof SelectionChangedEvent){
      if(event.dbIdArray.length > 0){
        ...
        console.error('event fired without any selected items, ignoring')
      }
    }
    if(event instanceof IsolateEvent){
      ...
      console.error('event fired without any isolated items, ignoring')
      }
